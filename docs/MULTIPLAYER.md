...
The menu polls `/api/matchmaking` every five seconds for actual connected random-queue players and public custom lobbies. Private lobbies are not listed or counted. Random Match and Quick Play join an available random lobby or create one with a server-selected rules preset. The first player triggers a 60-second countdown; it continues while at least one player remains. AI nations fill vacant slots up to eight human/AI participants, while retaining larger preset AI populations. Closing the lobby panel keeps its connection and shows Return to lobby; Leave explicitly exits. Starting single player or signing out closes the waiting connection. Custom Match opens its own browser with saved-world selection, AI counts, match length, gold income and nukes. Custom hosts start a five-second countdown after everyone is ready. Any player unreadying or leaving cancels that countdown. Host ownership transfers to the next player when a waiting host leaves. Game ticks still wait for all clients to finish loading.

`MAX_ROOMS` controls concurrent rooms (default 4; the small Render template retains 1 until capacity is measured). Random matches with at least two human participants qualify for rankings after five minutes; custom rules and private matches never do. Test with `node --env-file=.env.local --import ./scripts/register-tests.mjs scripts/multiplayer/matchmaking-check.mjs`; it checks two independent browsers, real live counts, countdown continuity, custom-map transfer and deterministic play, then deletes its temporary accounts.
