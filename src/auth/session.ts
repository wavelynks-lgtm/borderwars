...
export async function authToken(){
 if(!neonAuth)return localStorage.getItem('borderwars.session');
 // Neon 0.5 caches /token and /get-session under the same key. Force the
 // token request to reach the provider instead of returning cached user data.
 const {data,error}=await neonAuth.token({fetchOptions:{headers:{'X-Force-Fetch':'true'}}});
 if(error){if(error.status===401)return null;throw new Error(error.message??'Please sign in again');}
